...
class ImportCSV extends Import {
	...
	protected function parse_file() {
		...
		// Parse cells into CSV rows, then back into strings for the format expected in $this->file_data
		$cells = str_getcsv($this->file_data);
		...
		for($i = $numColumns; $i < count($cells); $i += $numColumns)
		{
			$row = array_slice($cells, $i, $numColumns);
			...
			$this->file_data[] = rtrim( stream_get_contents($fp), "\n" );
