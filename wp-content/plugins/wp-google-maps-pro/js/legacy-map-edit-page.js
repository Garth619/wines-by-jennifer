    (function($){
		...
		function wpgmza_bulk_delete()
		{
			...
			var data = {
				...
				map_id: wpgmza_legacy_map_edit_page_vars.map_id
			};
			...
			{
				$.post(ajaxurl, jQuery.extend({marker_id: id}, data), function(response) {
					if(--counter == 1)
					{
		...
		jQuery(document).ready(function(){
			jQuery(document.body).on("click", function(event) {
				if(jQuery(event.target).is(".wpgmza.bulk_delete"))
				{
					wpgmza_bulk_delete();
					return;
				}

				if(jQuery(event.target).is(".wpgmza.select_all_markers"))
				{
